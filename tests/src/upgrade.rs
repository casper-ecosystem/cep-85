...
use casper_types::{runtime_args, AddressableEntityHash, EntityAddr, Key};
use cep85::{
...
fn should_upgrade_and_update_account_contract_contexts() {
    ...
    let cep18_contract_hash_contract_version = builder.query(
        None,
        Key::AddressableEntity(EntityAddr::Account(DEFAULT_ACCOUNT_ADDR.value())),
        &[],
    );

    dbg!(cep18_contract_hash_contract_version);

    // let cep18_contract_hash_contract_version = builder
    //     .query(
    //         None,
    //         Key::AddressableEntity(EntityAddr::Account(DEFAULT_ACCOUNT_ADDR.value())),
    ...
    // assert_eq!(cep18_contract_hash_contract_version, 1_u32);
    ...
    // let upgraded_cep18_contract_hash: AddressableEntityHash = account
    //     .named_keys()
    ...
    // let contract = builder
    //     .get_entity_with_named_keys_by_entity_hash(upgraded_cep18_contract_hash)
    //     .expect("should have contract");
    ...
    // assert_eq!(cep18_contract_hash_contract_version, 2_u32);
}
...
fn should_emit_event_on_upgrade_with_events_mode_ces() {
    ...
    let actual_event: Upgrade = get_event(&mut builder, &cep18_contract_hash, 0);
