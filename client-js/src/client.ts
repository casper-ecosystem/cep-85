...
export default class Client {
  ...
  private readonly _events: Record<
    string,
    ((event: WithTransactionInfo<CEP85Event>) => void)[]
  > = {};
  ...
  public addEventListener(
    name: string,
    listener: (event: CEP85EventResult) => void
  ) {
  ...
  public removeEventListener(
    name: string,
    listenerToRemove: (event: CEP85EventResult) => void
  ) {
    ...
    const filterListeners = (listener: (event: CEP85EventResult) => void) =>
   ...
   * @param listener - The callback function to execute when the event occurs.
   */
  public on(name: string, listener: (event: CEP85EventResult) => void) {
        ...
        (await this.parseExecutionResult(executionResult))
          ?.map(
            (result) =>
              ...
              }) as unknown as CEP85EventResult
          )
  ...
  /**
   * Queries a contract dictionary item from the blockchain.
   *
   * @param key - The contract's key (e.g., contract hash) from which the dictionary is queried.
   * @param dictionaryName - The name of the dictionary within the contract.
   * @param dictionaryItemKey - The specific key within the dictionary to retrieve.
   * @returns A promise that resolves to the string representation of the stored value, or `undefined` if the item is not found.
   *
   * @throws Will throw an error if the query fails for reasons other than a missing record.
   */
  protected async queryContractDictionary(
    ...
    try {
      ...
      if (error instanceof Error && error.toString().includes('Query failed')) {
        console.warn(`No dict item found for ${dictionaryItemKey}`);
        return undefined;
      } else throw error;
    }
  }
  ...
  /**
   * Parses the execution result to extract CEP85 events.
   *
   ...
   * @returns A `Promise` resolving to an array of parsed CEP85 events or `undefined` if no valid events were found.
   */
  private async parseExecutionResult(
    result: ExecutionResult
  ): Promise<CEP85Event[] | undefined> {
   ...
   */
  private emit(event: CEP85EventResult) {
