import { CEP85Client, CEP85_EVENTS, CEP85EventResult } from 'src';
...
describe('CEP85Client - Event Streaming', () => {
  beforeEach(async () => {
  ...
  it('should start and stop event stream and listen to events when on() is called', async () => {
    ...
    await mint(client, id, mintAmount, waitForTransactionProcessed);

    // Listen for Mint event in a promise awaited
    await new Promise<CEP85EventResult>((resolve) => {
  ...
  it('should remove a specific event listener using off()', async () => {
    ...
    await mint(client, id, BigInt(10_000_000_000), waitForTransactionProcessed);
  ...
  it('should remove all event listeners using removeAllListeners()', async () => {
    ...
    client.on(CEP85_EVENTS.Mint, () => {
      mintTriggered = true;
    });

    client.on(CEP85_EVENTS.Burn, () => {
      burnTriggered = true;
    });
