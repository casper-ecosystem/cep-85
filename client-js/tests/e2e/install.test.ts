...
import { CEP85Client, TransactionParams, TransactionResult } from '../../src';
...
describe('CEP85Client - E2E Install', () => {
  beforeEach(() => {
    client = new CEP85Client(RPC_URL, SSE_URL, CHAIN_NAME);
  });

  it('should install the CEP85 contract and return valid transaction info', async () => {
  ...
  it('should install the CEP85 contract and return valid transaction result', async () => {
    const name = `TEST_CEP85_E2E_${Math.floor(Math.random() * 1000000)}`,
